...
$(document).ready(function () {
	...
	*/
	...
	// Init último slider index

	function initPoliticaSlider() {
		let politicaSlider = $('.politica_slider').owlCarousel({
			loop: false,
			center: true,
			mouseDrag: true,
			dots: true,
	...
	function initScrolling() {
		if ($('.nav_links').length) {
			...
			links.each(function () {
				...
				let parent = $('.main_nav ul li');;
				let clickedItemParent = ele.parent();
				ele.on('click', function (e) {
					e.preventDefault();
					if (window.innerWidth <= 991) {
						parent = $('.menu_mm ul li');
					}
					$(window).scrollTo(target, 1500, { offset: -80, easing: 'easeInOutQuart' });
					parent.removeClass('active');
					clickedItemParent.addClass('active');
				});
			});
		}
	}
